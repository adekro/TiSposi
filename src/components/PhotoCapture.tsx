...
const PhotoCapture = forwardRef<PhotoCaptureHandle, PhotoCaptureProps>(
  function PhotoCapture({ publicId, onUploaded, onError }, ref) {
      ...
      async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
          ...
          if (!ALLOWED_TYPES.includes(file.type)) {
            onError(
              `Formato non supportato: ${file.type}. Usa JPEG, PNG o WebP.`,
            );
            return;
          }
    ...
    const handleUpload = useCallback(async () => {
      ...
      if (!publicId) {
        onError("Evento non valido: impossibile caricare la foto.");
        return;
      }
    ...
    }, [publicId, selectedFile, onUploaded, onError]);
