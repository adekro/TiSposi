import type { VercelRequest, VercelResponse } from "@vercel/node";
import { getServiceSupabaseClient } from "./_lib/supabase.js";

export const config = {
  api: {
    bodyParser: false,
  },
};

const ALLOWED_MIME = new Set(["image/jpeg", "image/png", "image/webp"]);
const MAX_SIZE_BYTES = 4 * 1024 * 1024;

async function readRawBody(req: VercelRequest): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const chunks: Buffer[] = [];
    req.on("data", (chunk: Buffer) => chunks.push(chunk));
    req.on("end", () => resolve(Buffer.concat(chunks)));
    req.on("error", reject);
  });
}

function getBlockId(req: VercelRequest) {
  return typeof req.query.blockId === "string" ? req.query.blockId.trim() : "";
}

async function resolveOwnerEvent(
  supabase: ReturnType<typeof getServiceSupabaseClient>,
  authHeader: string | undefined,
) {
  if (!authHeader?.startsWith("Bearer ")) {
    return { event: null, status: 401, error: "Non autenticato" };
  }

  const token = authHeader.slice(7);
  const {
    data: { user },
    error: authErr,
  } = await supabase.auth.getUser(token);

  if (authErr || !user) {
    return { event: null, status: 401, error: "Token non valido" };
  }

  const { data: event, error: eventErr } = await supabase
    .from("events")
    .select("id")
    .eq("owner_user_id", user.id)
    .maybeSingle<{ id: string }>();

  if (eventErr || !event) {
    return { event: null, status: 404, error: "Evento non trovato" };
  }

  return { event, status: 200, error: null };
}

export default async function handler(req: VercelRequest, res: VercelResponse) {
  const blockId = getBlockId(req);
  if (!blockId) {
    return res.status(400).json({ error: "Parametro blockId mancante" });
  }

  let supabase;
  try {
    supabase = getServiceSupabaseClient();
  } catch (err) {
    const detail = err instanceof Error ? err.message : String(err);
    return res.status(500).json({ error: "Config server incompleta", detail });
  }

  if (req.method === "GET") {
    const eventId = typeof req.query.eventId === "string" ? req.query.eventId.trim() : "";
    if (!eventId) {
      return res.status(400).json({ error: "Parametro eventId mancante" });
    }

    const { data, error } = await supabase
      .from("event_landing_block_images")
      .select("image_base64, image_mime_type")
      .eq("event_id", eventId)
      .eq("block_id", blockId)
      .maybeSingle();

    if (error) {
      return res.status(500).json({ error: error.message });
    }
    if (!data) {
      return res.status(404).json({ error: "Nessuna immagine blocco" });
    }

    const imgBuffer = Buffer.from(data.image_base64 as string, "base64");
    res.setHeader("Content-Type", data.image_mime_type as string);
    res.setHeader("Cache-Control", "s-maxage=3600, stale-while-revalidate=86400");
    return res.status(200).send(imgBuffer);
  }

  if (req.method === "POST") {
    const { event, status, error } = await resolveOwnerEvent(
      supabase,
      req.headers.authorization,
    );
    if (!event) {
      return res.status(status).json({ error });
    }

    const contentType = (req.headers["content-type"] ?? "").split(";")[0].trim();
    if (!ALLOWED_MIME.has(contentType)) {
      return res.status(415).json({ error: `Tipo file non supportato: ${contentType}` });
    }

    const rawBody = await readRawBody(req);
    if (rawBody.length === 0) {
      return res.status(400).json({ error: "Nessun file ricevuto" });
    }
    if (rawBody.length > MAX_SIZE_BYTES) {
      return res.status(413).json({ error: "File troppo grande (max 4 MB)" });
    }

    const { error: upsertErr } = await supabase
      .from("event_landing_block_images")
      .upsert(
        {
          event_id: event.id,
          block_id: blockId,
          image_base64: rawBody.toString("base64"),
          image_mime_type: contentType,
          updated_at: new Date().toISOString(),
        },
        { onConflict: "event_id,block_id" },
      );

    if (upsertErr) {
      return res.status(500).json({ error: upsertErr.message });
    }

    return res.status(200).json({
      ok: true,
      url: `/api/upload-landing-block-image?eventId=${event.id}&blockId=${encodeURIComponent(blockId)}`,
    });
  }

  if (req.method === "DELETE") {
    const { event, status, error } = await resolveOwnerEvent(
      supabase,
      req.headers.authorization,
    );
    if (!event) {
      return res.status(status).json({ error });
    }

    const { error: delErr } = await supabase
      .from("event_landing_block_images")
      .delete()
      .eq("event_id", event.id)
      .eq("block_id", blockId);

    if (delErr) {
      return res.status(500).json({ error: delErr.message });
    }

    return res.status(200).json({ ok: true });
  }

  return res.status(405).json({ error: "Method not allowed" });
}
